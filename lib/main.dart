...
15.	class MyApp extends StatelessWidget { - Define a classe MyApp, que é um widget sem estado. Um widget sem estado é um widget que descreve parte da interface do usuário que pode depender de configuração, mas não depende do estado em execução.
16.	const MyApp({super.key}); - Este é o construtor para a classe MyApp. Ele aceita um argumento nomeado key que é passado para a superclasse StatelessWidget.
17.	@override - Esta anotação indica que a função build está substituindo uma função com o mesmo nome na superclasse.
18.	Widget build(BuildContext context) { - Define o método build, que retorna o widget que este widget apresentará.
19.	final themeModel = Provider.of<ThemeModel>(context); - Obtém a instância de ThemeModel do Provider.
